  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[var(--color-paper)] px-4">
      <div className="w-full max-w-sm bg-[var(--color-paper-raised)] border border-[var(--color-hairline)] rounded-lg p-8">
        <h1 className="text-2xl mb-1">Create your ledger</h1>
        <p className="text-sm text-[var(--color-ink-soft)] mb-6">
          Track income, expenses, and budgets — works offline once you've signed in.
        </p>

        {error && <p className="text-sm text-[var(--color-expense)] mb-4">{error}</p>}
...
function friendlyAuthError(err) {
  ...
  if (code.includes('network-request-failed')) return 'First sign-up requires an internet connection.'
